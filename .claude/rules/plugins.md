...
5. **Authentication is opt-in per route group.** `AuthPlugin` is _not_ in `baseApp`. Modules call `.use(AuthPlugin)` before the routes that need a logged-in user, leaving public routes above it.
